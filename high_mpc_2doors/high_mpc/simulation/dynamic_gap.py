...
from high_mpc.simulation.door_v2 import Door_v2
from high_mpc.simulation.door_v3 import Door_v3
#
...
class DynamicGap(object):
    ...
    def __init__(self, mpc, plan_T, plan_dt):
        ...
        # 
        self.goal_point = np.array([5.0,  0.0, 3.0])
        self.start_point = np.array([0.0, 0.0, 1.0])
        self.start2_point = np.array([3.0, 0.0, 1.0])
        ...
        self.door2 = Door_v2(self.start2_point, dt=self.sim_dt)

        self.planner = Door_v1(center_point=self.start_point, sigma=10, \
            T=self.plan_T, dt=self.plan_dt)
        
        self.planner2 = Door_v3(center_point=self.start2_point, sigma=10, \
            T=self.plan_T, dt=self.plan_dt)
    ...
    def reset(self, init_ypos=None):
        ...
        self.door2_state = self.door2.reset()
        ...
        door2_obs = self.door2.get_cartesian_state()
        #
    ...
    def step(self, u=0):
        ...
        plan_door_traj1, pred_door_traj_cart1 = self.planner.plan(self.door_state, opt_t)
        
        plan_door_traj2, pred_door_traj_cart2 = self.planner2.plan(self.door2_state, opt_t)
        

 

        if obs[0]>0.2:
            plan_door_traj = plan_door_traj2
            pred_door_traj_cart = np.array(pred_door_traj_cart2)
        else:
            plan_door_traj = plan_door_traj1
            pred_door_traj_cart = np.array(pred_door_traj_cart1)
        
        pred_door_traj_cart1 = np.array(pred_door_traj_cart1)
        pred_door_traj_cart2 = np.array(pred_door_traj_cart2)
        #
        ...
        self.door2_state = self.door2.run()
        ...
        info = {
            ...
            "door2_corners": self.door2.get_3d_corners(),
            "pred_quad_traj": pred_traj, 
            "pred_door_traj1": pred_door_traj_cart1, 
            "pred_door_traj2": pred_door_traj_cart2, 
    ...
    def episode(self, u):
        ...
        if obs[0]>0.2:
            ...
            plan_door_traj, pred_door_traj_cart = self.planner2.plan(self.door_state, opt_t)
            pred_door_traj_cart = np.array(pred_door_traj_cart)
