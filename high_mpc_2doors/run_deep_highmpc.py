...
def run_deep_high_mpc(env, actor_params, load_dir):
    ...
    for i in range(10):
        ...
        while t < env.sim_T:
            ...
            if obs[0]<0 and next_obs[0]>0 and count == 0:
            ...
            if obs[0]<0 and next_obs[0]>0 and count == 1:
                y=obs[0]/(-obs[0]+next_obs[0])*obs[1]+next_obs[0]/(-obs[0]+next_obs[0])*next_obs[1]
                z=obs[0]/(-obs[0]+next_obs[0])*obs[2]+next_obs[0]/(-obs[0]+next_obs[0])*next_obs[2]
                error = [np.sqrt(y**2+z**2)]
                aim_t = aim_t + t
                error_detect2.append(error)
            
            if -quad_length/2 < obs[0] < quad_length/2 and crash == 1:
                crash = judge(obs[1],obs[2])
                crash_times = crash_times + crash
    ...
    np.savetxt('error_door1.txt',error_detect1)
    np.savetxt('error_door2.txt',error_detect2)
    aim_t = aim_t/10
...
def main():
    ...
    if args.option == 0: # collect data
        ...
        load_dir = args.save_dir + "/Dataset/act_net/weights_1000.h5"
