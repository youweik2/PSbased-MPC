...
class DynamicGap(object):
    ...
    def __init__(self, mpc, plan_T, plan_dt):
        ...
        self.pivot_point = np.array([2.0, 0.0, 1.0])
    ...
    #    self.pend = Pendulum_v0(self.pivot_point, dt=self.sim_dt, rand_k=self.krandom)

        self.planner = Pendulum_v1(center_point=self.pivot_point, sigma=10, \
            T=self.plan_T, dt=self.plan_dt)
    #        T=self.plan_T, dt=self.plan_dt, rand_k=self.krandom)
    ...
    def reset(self, init_ypos=None):
        ...
        if init_ypos is not None:
            self.pend_state = self.pend.reset(init_ypos)
        else:
