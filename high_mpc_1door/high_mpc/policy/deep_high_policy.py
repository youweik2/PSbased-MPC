...
def train(env, logger, data_dir, hidden_units, learning_rate, \
    ...
    loss_result = np.zeros((3,train_epoch))
    ...
    for epoch in range(train_epoch):
        ...
        loss_result[0,epoch] = np.min(train_loss)
        loss_result[1,epoch] = np.mean(train_loss)
        loss_result[2,epoch] = np.max(train_loss)

        if (epoch) % 100 == 0:
            print("Epoch {:03d}: Loss: {:.3f}".format(epoch, np.mean(train_loss)))
            actor.save_weights(data_dir, iter=epoch)
    np.savetxt('beta=10.0.txt',loss_result)
